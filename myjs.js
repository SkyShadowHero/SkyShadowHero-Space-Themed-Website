...
function drawStarBackground(ctx, width, height) {
    ...
    gradientAngle += 0.01;
    // 渐变中心随时间缓慢移动
    let cx = width / 2 + Math.cos(gradientAngle) * width * 0.1;
    let cy = height / 2 + Math.sin(gradientAngle) * height * 0.1;
    ...
    radial.addColorStop(0.3, "#181c2a");    // 过渡
    radial.addColorStop(0.6, "#0a0c1b");    // 边缘更深
